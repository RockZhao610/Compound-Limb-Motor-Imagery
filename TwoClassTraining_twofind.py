# -*- coding: utf-8 -*-
"""
Created on Mon Jun  3 13:48:14 2024
...
def split_epochs(epochs, train_size=0.60, test_size=0.20):
    ...
    train_idx, test_valid_idx = train_test_split(indices, train_size=train_size, random_state=5)
...
################################################################################################################################################
# 加载数据
# data_path='D:/laboratory_zmm/MotorImagery/zyj/noICA/'
# RLLA_file = 'S1_2_56chs_-epo.fif'
# LLRA_file = 'S1_1_56chs_-epo.fif'

data_path='F:/张老师实验室/复合肢体运动想象/马芮备份文件/datamr/预处理后数据/MI/S3/'
RLLA_file = 'WC1T6_56chs_-epo.fif'
LLRA_file = 'WC1T5_56chs_-epo.fif'
...
# 增强训练数据
X_train_augmented, Y_train_augmented = augment_data(X_train, Y_train, augment_factor=3)

# 转换标签为分类格式
le = LabelEncoder()
y_train_augmented = to_categorical(le.fit_transform(Y_train_augmented), 2)
...
# 预定义模型
model = EEGNet_rz1(nb_classes=2, Chans=37, Samples=256, dropoutRate=0.5, kernLength=128, F1=8, D=2, F2=16, norm_rate=0.25, dropoutType='Dropout')
# 计算模型权重
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.8, patience=3, min_lr=8e-5)

# 新增的ModelCheckpoint回调，用于保存验证集上loss最低的模型
checkpoint_filepath = 'model_checkpointswc542/'
os.makedirs(checkpoint_filepath, exist_ok=True)
model_checkpoint = ModelCheckpoint(
    filepath=checkpoint_filepath + 'model-{epoch:02d}-{val_loss:.4f}.keras',
    save_best_only=True,
    monitor='val_loss',
    mode='min',
...
history = model.fit(x_train_augmented, y_train_augmented, epochs=300, batch_size=32, validation_data=(x_valid, y_valid), class_weight=class_weights, callbacks=[early_stopping, reduce_lr, model_checkpoint])
...
# # 加载模型
# model_path = 'D:/laboratory_zmm/MotorImagery/send/code/model_checkpointsZYJ542/model-74-0.3773.keras'
